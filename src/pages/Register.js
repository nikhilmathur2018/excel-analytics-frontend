...
import { register, reset } from '../features/auth/authSlice'; // Import register and reset from your authSlice
...
function Register() {
 ...
 const onChange = (e) => {
  setFormData((prevState) => ({
    ...prevState, // <-- Correct: Spreads all properties from the previous state
  ...
  const onSubmit = (e) => {
    ...
    if (password !== password2) {
      alert('Passwords do not match'); // Again, consider a better UI for this
    } else {
      const userData = {
        username,
        email,
      ...
      dispatch(register(userData)); // Dispatch the register async thunk
    }
  };
